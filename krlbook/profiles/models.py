...
class Profile(models.Model):
    ...
    def get_dislikes_given_no(self):
        dislikes = self.dislike_set.all()
        total_disliked = 0
        for item in dislikes:
            if item.value=='Dislike':
                total_disliked+= 1
        return total_disliked

    def get_dislikes_received_no(self):
        posts = self.posts.all()
        total_disliked = 0
        for item in posts:
            total_disliked += item.disliked .all().count()
        return total_disliked
    ...
    def save(self, *args, **kwargs):
        ...
        if self.first_name != self.__initial_first_name and self.last_name != self.__initial_last_name or self.slug=='':
            if self.first_name and self.last_name:
                ...
                while ex:
                    to_slug = slugify(to_slug + ' ' + str(create_random_code()))
                    ex = Profile.objects.filter(slug=to_slug).exists()
            else:
